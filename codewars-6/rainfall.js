// https://www.codewars.com/kata/rainfall/train/javascript 
...
function extractRawData(town, strng) {
  const singleTownData = strng.split('\n').filter(str => {
    return str.indexOf(town) > -1;
  }); 

if (!singleTownData.length) return -1;

  return singleTownData[0]
    .slice(town.length + 1)
    .split(',')
...
function mean(town, strng) {
  if (!town || typeof town !== 'string') return -1;
  const rawData =  extractRawData(town, strng);

  if (rawData === -1) return rawData;

  let total = 0;
  rawData.forEach(num => total += num);

  return total / rawData.length || -1;
}
...
function variance(town, strng) {
  ...
  if (avg === -1 || rawData === -1) return -1;

  let total = 0;
  
  rawData.forEach(data => {
    total += (avg - data) * (avg - data); 
  }); 
